...
- Archive 連結使用 CONFIG['base_path'] 絕對路徑，避免 /archive/archive/ 造成 404
"""
...
CONFIG = {
    ...
    # GitHub Pages project site 的固定 base path。
    # 若 repo 改名，請同步改這裡。例如 repo 名叫 my-site，就改成 '/my-site'
...
def fetch_feed(url, brand, query, provider):
    ...
    try:
        r=requests.get(url,headers={'User-Agent':'Mozilla/5.0 Top5LuxuryPlainListAgent/1.1'},timeout=10)
...
def fetch_articles():
    ...
    for brand in CONFIG['brands']:
        for tmpl in CONFIG['queries']:
            ...
            # Yahoo RSS 有時會回 500；保留為輔助來源，失敗時只會 warning，不影響網站生成。
...
def archive_url(filename):
    # 統一使用絕對路徑，避免在 /archive/ 頁面內再點擊時變成 /archive/archive/...
    return f"{CONFIG['base_path']}/archive/{filename}"
