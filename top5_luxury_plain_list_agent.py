...
CONFIG={
 ...
 'queries':['{brand} new collection','{brand} new arrivals','{brand} latest campaign','{brand} fashion news','{brand} product launch','{brand} runway collection'],
...
def report_date_str(): return now().strftime('%Y-%m-%d')
...
def parse_date(entry):
    ...
    return None
def local_date_from_pub(pub):
    if not pub: return ''
    try:
        return (pub.astimezone(dt.timezone.utc).replace(tzinfo=None)+dt.timedelta(hours=CONFIG['tz'])).strftime('%Y-%m-%d')
    except Exception:
        try: return (pub.replace(tzinfo=None)+dt.timedelta(hours=CONFIG['tz'])).strftime('%Y-%m-%d')
        except Exception: return ''
def include_by_report_date(pub):
    if not CONFIG.get('strict_report_date_only', True): return True
    return bool(pub) and local_date_from_pub(pub)==report_date_str()
...
def score_article(a):
    t=(a.get('title','')+' '+a.get('summary','')+' '+a.get('brand','')).lower(); score=0
...
def fetch_feed(url,brand,query,provider):
    out=[]
    try:
        r=requests.get(url,headers={'User-Agent':'Mozilla/5.0 Top5LuxuryStrictDateAgent/1.0'},timeout=10)
    ...
    for entry in feed.entries:
        ...
        if not include_by_report_date(pub): continue
        ...
        out.append({'id':article_id(title,source),'brand':brand,'title':title,'summary':summary,'source':source,'provider':provider,'query':query,'link':link,'published':pub.isoformat() if pub else '','published_local_date':local_date_from_pub(pub),'scraped_at':now().replace(microsecond=0).isoformat()})
        count+=1
    return out
...
def render_site(articles):
    ...
    (DOCS/'reports.json').write_text(json.dumps([{'date':n.replace('.html',''),'url':archive_url(n)} for n in unique],ensure_ascii=False,indent=2),encoding='utf-8')
def main():
    ...
    articles=fetch_articles(); date=report_date_str()
